...
export default function DosyaGaleri (props) {
  ...
  useEffect(()=>{
    ...
    //let defaultValue = props.values[props.compName + '_default'];
  ...
  const itemClick = (e, index) => {
    ...
    e.preventDefault();
    appContext.setModalData({...appContext.modalData, show: true, innerComp: 'ModalFileComp', uploadListData: props.uploadListData, selectedItem: index, compName: props.compName, uploadCompChange: props.uploadCompChange, uploadDeleteItem: props.uploadDeleteItem,uploadMakeDefault: props.uploadMakeDefault, listDefaultValue: props.values[props.compName + '_default'], setMyValueEmpty: props.setMyValueEmpty });
  ...
  const SortableItem = SortableElement(({value, sortIndex}) => {
    ...
    return(<div className={'item ' + (defaultValue == value.value ? 'defaultSelected' : '')}>
    ...
    <React.Fragment>
      <div className={props.compName}>
        <div className="dz-default dz-message " data="">
          <span className="us-btn us-btn-blue">Dosya Seçiniz</span>
        </div>
      </div>
